...
public final class DbNonfinals implements Violations {
    ...
    public DbNonfinals(final Db db, final String repo) {
        this.db = db;
        this.repo = repo;
    }
    ...
    public void clear() throws YsnpException {
        try {
            db.write(
                new JdSql(
                    "DELETE FROM allfinal_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
            db.write(
                new JdSql(
                    "UPDATE allfinal SET badgeUrl = '' WHERE repo = ?",
                    new StringArg(repo)
                )
            );
    ...
    public void add(final Violation violation) throws YsnpException {
        try {
            ...
            db.write(
                new JdSql(
                    "INSERT OR REPLACE INTO allfinal_violation VALUES(?, ?, ?)",
                    new StringArg(id),
                    new StringArg(repo),
    ...
    public int count() throws YsnpException {
        try (
            final QueryResult result = db.read(
                new JdSql(
                    "SELECT count(*) FROM allfinal_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
        ) {
            ...
            throw new YsnpException("Can not get the nonfinals count.", e);
        }
    }
}
