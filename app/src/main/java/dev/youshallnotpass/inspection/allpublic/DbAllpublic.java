package dev.youshallnotpass.inspection.allpublic;
...
public final class DbAllpublic implements Inspection {

    private final Db db;
    private final String id;

    public DbAllpublic(final Db db, final String id) {
        this.db = db;
        this.id = id;
    }
    ...
    public String badgeUrl() throws YsnpException {
        try (
            final QueryResult result = db.read(
                new JdSql(
                    "SELECT badgeUrl FROM allpublic WHERE id = ?",
                    new StringArg(id)
                )
            )
        ) {
            ...
            if (!rs.next()) {
                throw new YsnpException(
                    "allpublic does not exists for id " + id
                );
            }
    ...
    public void calcBadge() throws YsnpException {
        try (
            final QueryResult result = db.read(
                new JdSql(
                    "SELECT threshold FROM allpublic WHERE id = ?",
                    new StringArg(id)
                )
            )
        ) {
            ...
            if (!rs.next()) {
                throw new YsnpException(
                    new Sprintf(
                        "Can not find the allpublic with id '%s'.",
                        id
                    ).toString()
                );
            }
            ...
            db.write(
                new JdSql(
                    "UPDATE allpublic SET badgeUrl = ? WHERE id = ?",
                    new StringArg(
                        new YsnpBadge(
                            "allpublic",
            ...
            throw new YsnpException(
                new Sprintf(
                    "Can not calc the badge by allpublic '%s'",
                    id
                ).toString(),
                e
            );
    ...
    public void updateThreshold(final int threshold) throws YsnpException {
        try {
            db.write(
                new JdSql(
                    "UPDATE allpublic SET threshold = ? WHERE id = ?",
            ...
            throw new YsnpException(
                new Sprintf(
                    "Can not update the threshold in allpublic with id '%s'",
                    id
                ).toString(),
                e
            );
    ...
    public Violations violations() throws YsnpException {
        try (
            final QueryResult qr = db.read(
                new JdSql(
                    "SELECT repo FROM allpublic WHERE id = ?",
                    new StringArg(id)
                )
            )
        ) {
            ...
            if (!rs.next()) {
                throw new YsnpException(
                    "Could not find the allpublic by id " + id
                );
            }
            return new DbNonpublics(db, rs.getString("repo"));
        } catch (final JdException | SQLException e) {
            throw new YsnpException(
                "Could not get the allpublic info by id " + id,
                e
            );
        }
    }
