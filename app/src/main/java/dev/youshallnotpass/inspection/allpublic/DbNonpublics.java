...
public final class DbNonpublics implements Violations {

    private final Db db;
    private final String repo;

    public DbNonpublics(final Db db, final String repo) {
        this.db = db;
        this.repo = repo;
    }
    ...
    public void clear() throws YsnpException {
        try {
            db.write(
                new JdSql(
                    "DELETE FROM allpublic_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
            db.write(
                new JdSql(
                    "UPDATE allpublic SET badgeUrl = '' WHERE repo = ?",
                    new StringArg(repo)
                )
            );
            ...
            throw new YsnpException(
                new Sprintf(
                    "Can not clear the nonpublics for repo '%s'",
                    repo
                ).toString(),
                e
            );
    ...
    public void add(final Violation violation) throws YsnpException {
        try {
            ...
            db.write(
                new JdSql(
                    "INSERT OR REPLACE INTO allpublic_violation VALUES(?, ?, ?)",
                    new StringArg(id),
                    new StringArg(repo),
            ...
            throw new YsnpException(
                new Sprintf(
                    "Can not add the nonpublic '%s' in repo '%s'",
                    repo,
    ...
    public int count() throws YsnpException {
        try (
            final QueryResult result = db.read(
                new JdSql(
                    "SELECT count(*) FROM allpublic_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
        ) {
            ...
            throw new YsnpException("Can not get the nonpublics count.", e);
        }
    }
}
