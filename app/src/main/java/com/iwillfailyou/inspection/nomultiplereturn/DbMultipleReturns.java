...
public final class DbMultipleReturns implements Violations {

    private final Db db;
    private final String repo;

    public DbMultipleReturns(final Db db, final String repo) {
        this.db = db;
        this.repo = repo;
    }
    ...
    public void clear() throws IwfyException {
        try {
            db.write(
                new JdSql(
                    "DELETE FROM nomultiplereturn_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
            db.write(
                new JdSql(
                    "UPDATE nomultiplereturn SET badgeUrl = '' WHERE repo = ?",
                    new StringArg(repo)
                )
            );
            ...
            throw new IwfyException(
                new Sprintf(
                    "Can not clear the multiplereturns for repo '%s'",
                    repo
                ).toString(),
                e
            );
    ...
    public void add(final Violation violation) throws IwfyException {
        try {
            ...
            db.write(
                new JdSql(
                    "INSERT OR REPLACE INTO nomultiplereturn_violation VALUES(?, ?, ?)",
                    new StringArg(id),
                    new StringArg(repo),
            ...
            throw new IwfyException(
                new Sprintf(
                    "Can not add the multiplereturn '%s' in repo '%s'",
                    repo,
    ...
    public int count() throws IwfyException {
        try (
            final QueryResult result = db.read(
                new JdSql(
                    "SELECT count(*) FROM nomultiplereturn_violation WHERE repo = ?",
                    new StringArg(repo)
                )
            );
        ) {
            ...
            throw new IwfyException("Can not get the multiplereturn count.", e);
        }
    }
}
